...
public class ThreeSum15 {
    public static List<List<Integer>> threeSum(int[] nums) {
        ...
        // idea: first pointer i ranges from 0..nums.length - 2; i.e.., stops 3 elts
        // behind end
        // NOTE: (i < n-2) since two pointers in the way :)
        ...
        for (int i = 0; i < n - 2; ++i) {
            ...
            while (j < k) { // NO overlap, since one cond says "i != j, i != k, and j != k" (i.e., distinct)
                ...
                if (sum < targetSum) {
                    ...
                    // here, you need to do the same check for updating and seeing if any other
                    // solns in this window [i..end]
                    // it also needs to be a while, since you keep ++j-ing until you get a non-dupe
                    // (ie [...i..j.....k] -> [...i....j...k] -> etc.
                    ...
                    // i.e., you found a solution, so advance each ptr and see if you can find more
                    // in the zone
                    //
                    // keymap.set("i", "<C-BS>", "<C-W>", { silent = true, desc = "Delete previous word (Ctrl+Backspace)" })
                    //
                    // -- Map Ctrl+Backspace to delete the previous word in command-line mode
                    // keymap.set("c", "<C-BS>", "<C-W>", { silent = true, desc = "Delete previous word (Ctrl+Backspace)" })
                    //
                    j++;
                    k--;
                }
